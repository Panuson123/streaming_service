...
MODEL_PATH = "streaming_service-model.pkl" # ที่เก็บโมเดลที่ train ไว้
...
# -----------------------------
class StreamingServiceFeatures(BaseModel):
    service_date: date = Field(..., description="วันเดือนปี (YYYY-MM-DD)")

@app.route("/api/streaming_service", methods=["POST"])
def predict_streaming_service_price():
    try:
        # รับข้อมูล JSON จาก client
        data = request.get_json()

        # ตรวจสอบและแปลงข้อมูลด้วย Pydantic
        features = StreamingServiceFeatures(**data)

        # แปลงวันเดือนปีเป็น year และ month
        year = features.service_date.year
        month = features.service_date.month
